...
@pytest.fixture
def other_user(session: Session):  # Add type hint
    existing_user = session.query(User).filter(User.email == "rachel@gmail.com").first()
    if existing_user:
        # If user exists from a previous failed test run cleanup, use it
        yield existing_user
    else:
        ...
        user_oauth_account = OAuthAccount(
            ...
            account_email="rachel@gmail.com",
            access_token="access_token",
            refresh_token="refresh_token",
            expires_at=(
                datetime.datetime.now() + datetime.timedelta(days=30)
...
def workspace(session: Session, user: User):  # Add type hints
    ...
    existing_workspace = (
        session.query(Workspace).filter(Workspace.name == "Test Workspace").first()
    )
