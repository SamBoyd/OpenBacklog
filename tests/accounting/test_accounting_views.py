...
class TestCancelSubscriptionAndRefund:
    """Test cases for cancel subscription and refund endpoint."""

    def test_cancel_subscription_and_refund_success(
        self, test_client: TestClient, user: User, session: Session
    ):
        """Test successful subscription cancellation and refund."""
        with patch(
            "src.accounting.accounting_views.accounting_controller.cancel_subscription_with_refund"
        ) as mock_cancel_subscription_with_refund:
            mock_cancel_subscription_with_refund.return_value = (
            ...
            assert response.status_code == 200
            mock_cancel_subscription_with_refund.assert_called_once_with(user, ANY)
...
class TestCancelSubscriptionImmediately:
    """Test cases for cancel subscription immediately endpoint."""

    def test_cancel_subscription_immediately_success(
        self, test_client: TestClient, user: User, session: Session
    ):
        """Test successful subscription immediate cancellation."""

        user_account_details = user.account_details
        user_account_details.stripe_customer_id = "stripe_customer_id"
        user_account_details.subscription_cancel_at = datetime.now()
        ...
        with patch(
            "src.accounting.accounting_views.StripeService"
        ) as mock_stripe_service_cls:
            mock_subscription = Mock()
            mock_subscription.id = "subscription_id"
            mock_stripe_service = Mock()
            mock_stripe_service_cls.return_value = mock_stripe_service
            mock_stripe_service.find_active_subscription.return_value = (
                mock_subscription
            )
            mock_stripe_service.cancel_subscription.return_value = "subscription_id"
            ...
            mock_stripe_service.find_active_subscription.assert_called_once_with(
                "stripe_customer_id"
            )
