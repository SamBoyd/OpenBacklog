...
def test_can_reorder_initiative_list(browser_page: Page, onboard_test_user: None):
    ...
    expect(
        browser_page.get_by_role("button", name="This is a new Initiative I-")
    ).to_be_visible()
    ...
    browser_page.get_by_test_id("initiative-description-section").get_by_test_id(
        "title-input"
    ).click()
    browser_page.get_by_test_id("initiative-description-section").get_by_test_id(
        "title-input"
    ).fill("Let's update the description text\n\nWith some multiline text")
    browser_page.get_by_test_id("bottom-content").get_by_text(
        "This is a new Initiative"
    ).click()
    browser_page.get_by_role("textbox", name="Title of your initiative").fill(
        "This is a new Initiative - updated title"
    )
    ...
    expect(
        browser_page.get_by_role(
            "button", name="This is a new Initiative - updated title"
        )
    ).to_be_visible()
    ...
    ).to_have_text("Let's update the description text\n\nWith some multiline text")
    expect(
        browser_page.get_by_test_id("bottom-content").get_by_text(
            "This is a new Initiative - updated title"
        )
    ...
    browser_page.get_by_test_id("expanded-form").get_by_test_id("title-input").fill(
        "This is a test task"
    )
    browser_page.get_by_role("button", name="To do").click()
    browser_page.get_by_role("option", name="In progress").click()
    browser_page.get_by_role("button", name="Select type").click()
    browser_page.get_by_test_id("type-select").get_by_role(
        "option", name="Bugfix"
    ).click()
    browser_page.get_by_test_id("create-button").click()
    expect(
        browser_page.get_by_role("button", name="TM-001 This is a test task")
    ).to_be_visible()

    # Navigate to task
    browser_page.get_by_role("button", name="TM-001 This is a test task").click()
    expect(
        browser_page.get_by_test_id("content").get_by_text("This is a test task")
    ).to_be_visible()
    ...
    browser_page.get_by_role("textbox", name="Title of your task").fill(
        "This is a test task - updated title"
    )

    # Go back and check the list in the initiative view
    browser_page.get_by_test_id("navigation-button").click()
    expect(
        browser_page.get_by_role("button", name="This is a test task - updated title")
    ).to_be_visible()

    # Navigate to task and check the values there
    browser_page.get_by_role(
        "button", name="This is a test task - updated title"
    ).click()
    expect(
        browser_page.get_by_test_id("content").get_by_text(
            "This is a test task - updated title"
        )
    ).to_be_visible()
    expect(browser_page.get_by_test_id("title-input")).to_have_text(
        "Let's update the task description"
    )
