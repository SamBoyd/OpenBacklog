...
class ImmediateCancellationResult(BaseModel):
    """Response model for immediate subscription cancellation."""

    success: bool = Field(description="Whether the cancellation was successful")
    message: str = Field(description="Success or error message")
    refund_amount_cents: int = Field(
        alias="refundAmountCents", default=0, description="Amount refunded in cents"
    )


@app.post(
    "/api/accounting/cancel-subscription-immediately",
    response_model=ImmediateCancellationResult,
)
async def cancel_subscription_immediately(
    ...
    """
    Cancel user's subscription immediately with full refund of last payment.

    This endpoint cancels the user's active subscription right away and processes
    a refund for their most recent payment, instead of waiting for the scheduled
    cancellation at the end of the billing period.
    """
    logger.info(f"Processing immediate subscription cancellation for user {user.id}")

    user_account_details = db.merge(user.account_details)

    if not user_account_details.stripe_customer_id:
        logger.exception(
            f"Exception processing immediate subscription cancellation for user {user.id}: User has no Stripe customer ID"
        )
        return ImmediateCancellationResult(
            success=False, message="User has no Stripe customer ID", refundAmountCents=0
        )

    stripe_service = StripeService(db)
    subscription = stripe_service.find_active_subscription(
        user_account_details.stripe_customer_id
    )
    if not subscription:
        logger.exception(
            f"Exception processing immediate subscription cancellation for user {user.id}: User has no active subscription"
        )
        return ImmediateCancellationResult(
            success=False,
    ...
    stripe_service.cancel_subscription(subscription.id)
    # Refund of the last subscription payment is handled by the stripe webhook handler

    # Clear pending cancellation fields since subscription is now immediately canceled
    ...
    return ImmediateCancellationResult(
        success=True,
        message="Subscription canceled successfully. Refund has been processed.",
        refundAmountCents=0,
    )


@app.post(
    "/api/accounting/cancel-subscription-and-refund",
    response_model=SubscriptionCancellationResult,
)
async def cancel_subscription_and_refund(
    user: User = Depends(dependency_to_override),
    db: Session = Depends(get_db),
) -> SubscriptionCancellationResult:
    return accounting_controller.cancel_subscription_with_refund(user, db)
